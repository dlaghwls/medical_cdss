...
const LabResultsView = ({ selectedPatient, onBackToPatientList }) => {
    ...
    const [sod2Records, setSod2Records] = useState([]);
    const [complicationsRecords, setComplicationsRecords] = useState([]);
...
const handleSubmit = async (e) => {
        ...
        if (activeForm === 'sod2') {
            ...
            if (!notes) { // 비고가 없으면 자동 생성
                submissionData.notes = generateSod2Note(submissionData, sod2Records);
            }
            ...
            if (!notes) { // 비고가 없으면 자동 생성
                submissionData.notes = generateComplicationsNote(submissionData, complicationsRecords);
            }
        } else if (activeForm === 'mortality') {
            submissionData = {
                patient_uuid: selectedPatient.uuid, gender: gender, age: parseInt(age),
                vital_signs: { heart_rate: parseFloat(heartRate), systolic_bp: parseFloat(systolicBp), diastolic_bp: parseFloat(diastolicBp), temperature: parseFloat(temperature), respiratory_rate: parseFloat(respiratoryRate), oxygen_saturation: parseFloat(oxygenSaturation) },
                lab_results: { wbc: parseFloat(wbc), hemoglobin: parseFloat(hemoglobin), creatinine: parseFloat(creatinine), bun: parseFloat(bun), glucose: parseFloat(glucose), sodium: parseFloat(sodium), potassium: parseFloat(potassium) },
                recorded_at: recordedAt
            };
        } else if (activeForm === 'lis') {
            submissionData = {
                patient_uuid: selectedPatient.uuid, test_name: testName, test_value: parseFloat(testValue), unit, notes, recorded_at: recordedAt
            };
        }
        ...
        try {
            if (activeForm === 'sod2') {
                // 로컬 상태(가상 DB)에 저장
                setSod2Records(prevRecords => [...prevRecords, submissionData]);
            } else if (activeForm === 'complications') {
                // 로컬 상태(가상 DB)에 저장
                setComplicationsRecords(prevRecords => [...prevRecords, submissionData]);
            } else if (activeForm === 'mortality') {
                // 실제 API 호출 (예시)
                console.log("Submitting to registerMortalityPredictionData:", submissionData);
                await new Promise(resolve => setTimeout(resolve, 500)); // 실제 API 함수로 교체 필요
            } else if (activeForm === 'lis') {
                // 실제 API 호출
                await registerLabResult(submissionData);
            }
            ...
            setError("데이터 등록 실패: " + (err.response?.data?.error || err.message || '알 수 없는 오류'));
        } finally {
            setLoading(false);
        }
    };
