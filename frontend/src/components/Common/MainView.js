...
import { uploadDicomFile } from '../../services/orthancApiService';
...
const MainView = ({ currentViewId, user }) => {
  ...
  switch (currentViewId) {
    ...
    case 'pacs_viewer':
          content = (
            <div>
              <h4>PACS 이미지 뷰어 및 업로드</h4>
              <input type="file" accept=".dcm" onChange={async (e) => {
                const file = e.target.files[0];
                if (file) {
                  try {
                    await uploadDicomFile(file);
                    alert('DICOM 파일 업로드 성공!');
                  } catch (err) {
                    alert('DICOM 파일 업로드 실패!');
                 }
               }
             }} />
           </div>
