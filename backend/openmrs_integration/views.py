...
from .utils import perform_openmrs_patient_sync # 이 함수가 정의되어 있다고 가정

# Django settings에서 OpenMRS 관련 설정값 가져오기
# getattr의 세 번째 인자는 settings에 해당 값이 없을 경우 사용할 기본값입니다.
...
PHONE_NUMBER_ATTRIBUTE_TYPE_UUID = getattr(settings, 'OPENMRS_PHONE_ATTRIBUTE_TYPE_UUID', None) # 이 설정도 settings.py나 .env에 정의 필요

# 로거 설정 (openmrs_integration 앱에 대한)
logger = logging.getLogger('openmrs_integration') # settings.py의 LOGGING 설정과 일치
...
def is_openmrs_uuid_valid(resource_type, uuid_to_check):
    ...
    # UUID 형식 자체를 간단히 검사 (실제 OpenMRS API 호출 전에)
    try:
...
def get_django_patient_list_only(request):
    ...
    try:
        limit = int(request.GET.get('limit', settings.OPENMRS_PATIENT_LIST_DEFAULT_LIMIT)) # settings 값 사용
    ...
    try:
        ...
        if query:
            logger.debug(f"get_django_patient_list_only: Applying filter for query: '{query}'")
            try:
        ...
        patients_qs_ordered = base_qs.order_by('display_name') # 혹은 다른 기준으로 정렬
        ...
        logger.debug(f"get_django_patient_list_only: Found {total_patients_after_filter} patients, displaying {len(list(current_patients_to_display))}.")
        ...
        logger.info(f"get_django_patient_list_only: Returning {len(patients_data)} patients. Total after filter: {total_patients_after_filter}")
        return Response({'results': patients_data, 'totalCount': total_patients_after_filter})
    except Exception as e:
        logger.exception(f"get_django_patient_list_only: Error - {type(e).__name__}: {e}") # 스택 트레이스 포함 로깅
...
def get_patients_and_sync_from_openmrs(request):
    ...
    default_sync_query = getattr(settings, 'DEFAULT_OPENMRS_SYNC_QUERY', "100") # 기본값 조정
    sync_query_from_request = request.GET.get('sync_q', default_sync_query)
    sync_limit = int(request.GET.get('sync_limit', 50))
    sync_max = int(request.GET.get('sync_max', 200)) # 최대 동기화 개수 제한
    ...
    try:
        ...
        def view_logger(message, style_func_name=None): logger.info(f"[SYNC_UTIL_IN_VIEW] {message}") # 로거 사용
        
        # perform_openmrs_patient_sync 함수가 정의되어 있다고 가정합니다.
        # 이 함수는 OpenMRS API를 호출하여 환자 정보를 가져오고 Django DB에 저장/업데이트합니다.
        # 이 함수 내부에서도 상세한 로깅이 필요합니다.
        ...
        logger.exception(f"get_patients_and_sync_from_openmrs: {sync_error_message}") # 스택 트레이스 포함
        sync_error_detail = sync_error_message

    # 동기화 후 로컬 DB에서 다시 목록 조회 (이전 로직과 유사하게)
    query_for_list = request.GET.get('q', '') # React에서 검색어 입력 시 사용
    ...
    try:
        ...
        for p_model in patients_to_display:
            # 이전 get_django_patient_list_only와 동일한 데이터 변환 로직 사용
            display_name_final = "정보 없음"; person_data_final = {}; identifiers_final = []
            if p_model.raw_openmrs_data and isinstance(p_model.raw_openmrs_data, dict):
                raw = p_model.raw_openmrs_data; display_name_final = raw.get('display', display_name_final)
                raw_ids = raw.get('identifiers', []);
...
def create_patient_in_openmrs_and_django(request):
    ...
    # Location UUID 확인 (필수적인지 여부는 OpenMRS 설정에 따라 다를 수 있음)
    if not location_uuid_from_settings or \
       not is_openmrs_uuid_valid('location', location_uuid_from_settings):
        # Location이 필수가 아니라면 이 부분을 경고로 처리하거나, OpenMRS API가 허용한다면 None으로 보낼 수 있습니다.
        # 여기서는 일단 오류로 처리합니다.
    ...
    # Phone Attribute Type UUID 확인 (전화번호가 제공된 경우에만)
    if data.get('phoneNumber') and phone_attr_type_uuid_from_settings and \
       not is_openmrs_uuid_valid('personattributetype', phone_attr_type_uuid_from_settings):
        ...
        # 전화번호 속성 UUID가 유효하지 않으면, 전화번호를 속성으로 추가하지 않고 진행할 수 있도록 처리 (선택적)
        # 여기서는 경고만 남기고, payload에서 전화번호 속성 부분을 제외할 수 있습니다.
        # 혹은 오류로 처리: return Response({'error': error_msg, ...})
        phone_attr_type_uuid_from_settings = None # 유효하지 않으므로 사용 안 함
    ...
    try:
        ...
        openmrs_payload = {
            "person": {
                ...
                "gender": gender,
                "birthdate": birthdate_str, # YYYY-MM-DD 형식으로 가정
                "addresses": [{"address1": address1, "cityVillage": city_village, "preferred": True}] if address1 or city_village else [],
                "attributes": []
            },
            "identifiers": [{
                "identifier": identifier_value,
                "identifierType": identifier_type_uuid_from_settings, # settings에서 가져온 UUID 사용
                "location": location_uuid_from_settings,             # settings에서 가져온 UUID 사용
                "preferred": True
            }]
        }

        if phone_number_str and phone_attr_type_uuid_from_settings: # 유효한 경우에만 추가
        ...
        # 응답 내용을 로깅할 때는 민감한 정보가 포함될 수 있으므로 주의합니다.
        # logger.debug(f"OpenMRS API response text for patient creation (first 1000 chars): {omrs_response.text[:1000]}")
        
        omrs_response.raise_for_status() # 2xx가 아니면 HTTPError 발생
        ...
        try:
            ...
            patient_obj, created_in_django = OpenMRSPatient.objects.update_or_create(
                ...
                defaults={
                    ...
                    'raw_openmrs_data': created_openmrs_patient_data # 전체 JSON 응답 저장
                }
            )
...
def get_openmrs_patient_detail(request, patient_uuid):
    ...
    try:
        ...
        api_url = f"{OPENMRS_API_BASE_URL}/patient/{valid_uuid}?v=full" # v=full로 상세 정보 요청
        ...
        response_from_omrs.raise_for_status() # 2xx가 아니면 HTTPError 발생
        ...
        try:
            ...
            OpenMRSPatient.objects.update_or_create(
                ...
                defaults={
                    ...
                    'raw_openmrs_data': openmrs_patient_data # 전체 JSON 응답 저장
                }
            )
            logger.info(f"Patient {valid_uuid} data saved/updated in Django DB.")
        except Exception as db_error:
            logger.exception(f"Error saving/updating patient {valid_uuid} to Django DB: {type(db_error).__name__} - {db_error}")
